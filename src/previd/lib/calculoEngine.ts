import { FormData } from "@previd/contexts/CalculoContext";
...
function getCorrecaoTotal(
  ...
  let igpDiFactor = 1.0;
  let inpcFactor = 1.0;
  let selicSimplesPercent = 0; // SELIC simples (Res. 448/2022 CNJ)
  let ipcaFactor = 1.0;
  ...
  // SELIC: acumulação simples (Res. 448/2022 CNJ, Manual de Cálculos JF)
  // Soma das taxas mensais do mês posterior ao vencimento até o mês anterior ao pagamento
  const selicStartMonth = new Date(fromDate.getFullYear(), fromDate.getMonth() + 1, 1); // mês posterior ao vencimento
  const selicEndMonth = new Date(toDate.getFullYear(), toDate.getMonth(), 1); // mês anterior ao pagamento (loop < endMonth)
  ...
  while (current < endMonth) {
    ...
    if (indice === "igp_di") {
      ...
      // SELIC simples: soma das taxas mensais (não composta)
      // Acumula do mês posterior ao vencimento até o mês anterior ao pagamento
  ...
  // Adiciona 1% referente ao mês do pagamento (Manual de Cálculos JF)
  if (selicSimplesPercent > 0 || getIndiceParaCompetencia(toDate.getFullYear(), toDate.getMonth()) === "selic") {
    selicSimplesPercent += 1.0;
  }

  // Converter SELIC simples para fator multiplicador
