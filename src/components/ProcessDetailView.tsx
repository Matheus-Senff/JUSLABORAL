...
export const ProcessDetailView: React.FC<ProcessDetailViewProps> = ({
  ...
  const handleSaveNote = () => {
    if (!noteForm.titulo.trim()) return
    ...
    const hist: ProcessHistoryEntry = {
      ...
      processId: process.id,
      tipo: 'auditoria',
      texto: `Anotação adicionada: ${note.titulo}`,
      autor: process.responsavel,
      data: new Date().toLocaleString('pt-BR'),
    }
    setHistory(prev => [hist, ...prev])
      ...
      <div className={`${card} border-b ${border} px-6 py-4 flex items-center justify-between sticky top-0 z-10`}>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition ${darkMode ? 'bg-dark-700 hover:bg-dark-600 text-yellow-400' : 'bg-yellow-50 hover:bg-yellow-100 text-yellow-700 border border-yellow-200'}`}
          >
            <StickyNote size={14} /> Anotação
          </button>
          <button
        ...
        <div className="flex gap-0">
          {tabs.map(t => (
            <button
              ...
              className={`px-4 py-3 text-sm font-medium border-b-2 transition ${activeTab === t.id
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
                  ...
                  {notes.map(note => (
                    <div key={note.id} className={`p-4 rounded-lg border ${border}`}>
                      <div className="flex items-center justify-between mb-2">
                        <p className={`font-semibold text-sm ${text}`}>{note.titulo}</p>
        ...
        {activeTab === 'historico' && (
          <div className={`${card} rounded-xl border ${border} p-6`}>
            ...
            <div className={`flex gap-2 mb-6 p-4 rounded-lg ${darkMode ? 'bg-dark-700' : 'bg-gray-50'}`}>
              <input
                type="text"
                value={newComment}
                onChange={e => setNewComment(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleAddComment()}
                placeholder="Adicionar comentário ou anotação de auditoria..."
                className={inputCls}
              />
              <button
                onClick={handleAddComment}
                disabled={!newComment.trim()}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition disabled:opacity-40 flex items-center gap-1.5"
              >
          ...
          <div className={`${card} rounded-xl shadow-2xl border ${border} w-full max-w-md`}>
            ...
            <div className={`flex gap-2 p-5 border-t ${border}`}>
              ...
              <button
                ...
                className="flex-1 px-4 py-2 rounded-lg bg-yellow-600 hover:bg-yellow-700 text-white text-sm font-medium transition disabled:opacity-40 flex items-center justify-center gap-1.5"
              >
                <Save size={14} /> Salvar
              </button>
            </div>
