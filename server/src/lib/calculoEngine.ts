// ======== MOTOR DE CÁLCULO PREVIDENCIÁRIO (SERVER) ========
// Cálculo estritamente conforme:
// - IGP-DI (Lei 9.711/98) até 08/2006
// - INPC (Lei 8.213/91, STJ Tema 905) de 09/2006 a 11/2021
// - SELIC simples (EC 113/2021, Res. 448/2022 CNJ) a partir de 12/2021 (+1% mês pgto)
// - Precatórios: IPCA (EC 136/2025) + Juros 2% a.a.
...
// ======== DETERMINAÇÃO DO ÍNDICE POR PERÍODO ========

type IndiceAplicavel = 'igp_di' | 'inpc' | 'selic';

function getIndiceParaCompetencia(year: number, month: number): IndiceAplicavel {
  // IGP-DI até 08/2006, INPC de 09/2006 a 11/2021, SELIC depois
  const ym = year * 100 + (month + 1);
  if (ym <= 200608) return 'igp_di';
  if (ym <= 202111) return 'inpc';
  return 'selic';
}
...
function getCorrecaoTotal(
  ...
  // SELIC simples (EC 113/2021, Res. 448/2022 CNJ)
  const selicStartMonth = new Date(fromDate.getFullYear(), fromDate.getMonth() + 1, 1);
  const selicEndMonth = new Date(toDate.getFullYear(), toDate.getMonth(), 1);
  ...
  while (current < endMonth) {
    ...
    if (indice === 'igp_di') {
      igpDiFactor *= 1 + taxa / 100;
    } else if (indice === 'inpc') {
      inpcFactor *= 1 + taxa / 100;
    } else {
      if (current >= selicStartMonth && current < selicEndMonth) {
        selicSimplesPercent += taxa;
      }
    }
  ...
  selicSimplesPercent += 1.0;

  const selicFactor = 1 + selicSimplesPercent / 100;
  ...
  return {
    ...
    selicFactor, selicPercent: selicSimplesPercent,
    ipcaFactor: 1, ipcaPercent: 0,
    indiceNome: selicSimplesPercent > 0 ? 'IGP-DI → INPC → SELIC' : '—',
  };
}
...
function calcularJurosMoratorios(
  ...
  // Juros de mora apenas para precatórios (2% a.a. pro rata)
  if (modoPrecatorio) {
    ...
    const percentual = (2 / 12) * meses;
    return { percentual, valor_sobre_principal: percentual / 100 };
  }
  // Fora precatório, não há juros de mora após EC 113/2021
  return { percentual: 0, valor_sobre_principal: 0 };
}
...
export function executarCalculo(input: CalculoInput): ResultadoCalculo {
  const inicio = parseDate(input.dataInicioCalculo);
  const termino = parseDate(input.dataTerminoCalculo);
  const atualizacao = parseDate(input.dataAtualizacao) || termino;
  const dataCitacao = parseDate(input.dataCitacao);

  if (!inicio || !termino) {
    throw new Error('Datas de início e término são obrigatórias');
  }

  const rmiBase = input.fixarSalarioMinimo ? getSalarioMinimo(inicio.getFullYear()) : parseCurrency(input.rmi);

  if (rmiBase <= 0 && !input.fixarSalarioMinimo) {
    throw new Error('RMI deve ser informada ou fixar em Salário Mínimo');
  }

  const modoPrecatorio = input.modoPrecatorio || false;
  ...
  while (current <= endDate) {
    ...
    const valorMensal = getValorReajustado(
      ...
      input.sistematicaReajuste, input.fixarSalarioMinimo,
    );
    ...
    parcelas.push(criarParcela(
      ...
      atualizacao, dataCitacao, input.correcaoMonetaria,
      'mensal', modoPrecatorio, diasTrab,
    ));

    if (input.calcular13Dezembro && month === 11 && !previousDecemberDone.has(year)) {
      previousDecemberDone.add(year);
      const valor13 = getValorReajustado(rmiBase, anoBaseRMI, year, input.sistematicaReajuste, input.fixarSalarioMinimo);
      parcelas.push(criarParcela(
    ...
    if (isUltimoMes && month !== 11 && input.incluir13UltimoAno && !previousDecemberDone.has(year)) {
      const mesesTrabalhados = month + 1;
      const valorBase13 = getValorReajustado(rmiBase, anoBaseRMI, year, input.sistematicaReajuste, input.fixarSalarioMinimo);
      const proporcional = input.integral13UltimoAno ? valorBase13 : (valorBase13 / 12) * mesesTrabalhados;
      parcelas.push(criarParcela(
  ...
  if (input.incluir12Vincendas) {
    for (let i = 1; i <= 12; i++) {
      ...
      const valorVinc = getValorReajustado(rmiBase, anoBaseRMI, vincYear, input.sistematicaReajuste, input.fixarSalarioMinimo);
      parcelas.push(criarParcela(
    ...
    if (input.incluir13Vincendas) {
      const ultimoAno = termino.getFullYear();
      const ultimaRMI = getValorReajustado(rmiBase, anoBaseRMI, ultimoAno, input.sistematicaReajuste, input.fixarSalarioMinimo);
  ...
  if (input.correcaoMonetaria) {
    if (modoPrecatorio) {
      ...
      if (totalJurosSelic > 0) partes.push('SELIC simples (EC 113/2021, Res. 448/2022 CNJ, a partir de 12/2021 + 1% mês pgto.)');
  ...
  return {
    ...
    especie: input.especie || 'Não informada',
    rmi: rmiBase,
    periodoInicio: input.dataInicioCalculo,
    periodoFim: input.dataTerminoCalculo,
    correcaoAplicada: correcaoStr,
    indiceCorrecaoNome: modoPrecatorio ? 'IPCA' : 'IGP-DI → INPC → SELIC',
    sistematicaReajuste: input.sistematicaReajuste === 'judiciario' ? 'Judiciário (INPC)' : 'INSS (Portarias MPS)',
    incluiu12Vincendas: input.incluir12Vincendas,
    incluiu13Vincendas: input.incluir13Vincendas,
    limitou60SM: input.limitar60SM,
    valorCausa, teto60SM, modoPrecatorio,
    notaRodape: 'Cálculo atualizado conforme EC 113/2021 e Resolução 448/2022 do CNJ (Taxa SELIC).',
  };
}
